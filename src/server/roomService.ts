...
export class RoomService {
  ...
  createSoloRoom(socketId: string, rawName: string): ServiceResult {
    ...
    const playerId = this.allocatePlayerId();
    const issuedAt = Date.now();
    const room: RoomRecord = { roomCode, phase: 'lobby', createdAt: issuedAt, version: 1, match: null, countdown: null, teardownAt: null,
  ...
  disconnect(socketId: string): ServiceResult { const room = this.getRoomForSocket(socketId); return this.leaveCurrentRoom(socketId, room?.soloMode ? 'left' : 'disconnected'); }
  ...
  private afterLobbyMutation(room: RoomRecord): ClientEvent[] { const events = this.buildLobbyStateEvents(room); if (!this.canStart(room)) return events; room.match = createInitialMatchState(room.roomCode, Math.random, room.soloMode); room.version += 1; this.clearRematchState(room); events.push(...this.beginCountdown(room, 3000)); return events; }
