...
export class RoomService {
  ...
  createRoom(socketId: string, rawName: string): ServiceResult {
    ...
    const room: RoomRecord = { roomCode, phase: 'waiting-for-players', createdAt: issuedAt, version: 1, match: null, countdown: null, teardownAt: null,
      rematch: { requestedBySlot: { 0: false, 1: false } }, disconnectWindow: null, pausedMatch: null,
      players: [
  ...
  private buildReconnectView(room: RoomRecord, viewerSocketId?: string): ReconnectView { const viewer = viewerSocketId ? room.players.find((slot) => slot.socketId === viewerSocketId) ?? null : null; const window = room.disconnectWindow; const secondsRemaining = window ? Math.max(0, Math.ceil((window.expiresAt - Date.now()) / 1000)) : null; return { active: Boolean(window), status: window ? (room.pausedMatch && room.players[window.slotIndex].connected ? 'resume-countdown' : 'waiting-for-player') : 'none', disconnectedSlotIndex: window?.slotIndex ?? null, disconnectedPlayerName: window ? room.players[window.slotIndex].name : null, reservedUntil: window?.expiresAt ?? null, secondsRemaining, affectedPhase: window?.affectedPhase ?? null, yourSlotReserved: Boolean(viewer && window && viewer.slotIndex === window.slotIndex), canAutoResume: Boolean(viewer && window && viewer.slotIndex === window.slotIndex) }; }
