...
type ClientEventType =
  ...
  | 'game:ended' | 'game:rematch-state' | 'game:room-transition' | 'room:closed' | 'session:issued' | 'session:resume:succeeded' | 'session:resume:failed';
...
export class RoomService {
  ...
  private scheduleNextTick(roomCode: string, delayMs: number) {
    ...
    runtime.tickTimer = setTimeout(() => {
      ...
      if (room.match.result) {
        ...
        const run = match.run;
        // Co-op run room transition: advance to next room instead of ending
        if (match.result === 'co-op-win' && run && run.currentRoom < run.totalRooms) {
          const transitionMs = 2500;
          const nextRoom = run.currentRoom + 1;
          const nextDifficulty = runDifficultyValue({ ...run, currentRoom: nextRoom });
          const events: ClientEvent[] = [];
          for (const player of room.players) {
            if (player.socketId) {
              events.push({
                type: 'game:room-transition',
          ...
          // Schedule the actual room advance after the transition delay
          const advanceTimer = setTimeout(() => {
            const activeRoom = this.rooms.get(roomCode);
            if (!activeRoom || activeRoom.phase !== 'in-progress' || !activeRoom.match || activeRoom.pausedMatch || activeRoom.disconnectWindow) return;
            const nextMatch = advanceCoOpRoom(activeRoom.match, Math.random);
            if (!nextMatch) {
              this.emitExternal(this.finishMatch(activeRoom, activeRoom.match, 'round-complete'));
              return;
            }
            activeRoom.match = nextMatch;
            activeRoom.version += 1;
            this.emitExternal(this.buildGameStateEvents(activeRoom));
            this.scheduleNextTick(roomCode, nextMatch.tickIntervalMs);
          }, transitionMs);
          runtime.countdownTimers.push(advanceTimer);
          return;
        }
  ...
  private getLobbyMessage(room: RoomRecord): string { if (room.disconnectWindow) { const player = room.players[room.disconnectWindow.slotIndex]; const seconds = Math.max(0, Math.ceil((room.disconnectWindow.expiresAt - Date.now()) / 1000)); const text = formatPlayerReference(player, { disambiguateWith: room.players.filter((entry) => entry.slotIndex !== player.slotIndex).map((entry) => entry.name) }); return room.pausedMatch ? `${text} disconnected. Waiting ${seconds}s for reconnect.` : `${text} disconnected. Slot reserved for ${seconds}s.`; } if (room.phase === 'starting' && room.countdown) return `Match starts in ${room.countdown.secondsRemaining}…`; if (room.phase === 'in-progress') return this.isCoOpRoom(room) ? (room.match?.run ? `Room ${room.match.run.currentRoom}/${room.match.run.totalRooms}. ${room.match.coOp?.monsters?.length ? 'Patrol room! Watch the monsters.' : room.match.coOp?.hazards?.length ? 'Hazard room! Watch the danger zones.' : room.match.coOp?.switches?.length ? 'Puzzle room. Coordinate switches.' : 'Reach the exit together.'}` : (room.match?.coOp?.monsters?.length ? 'Patrol room! Watch the moving monsters and time your crossings.' : room.match?.coOp?.hazards?.length ? 'Hazard room! Watch the danger zones and sweeper traps.' : room.match?.coOp?.switches?.length ? 'Co-op puzzle room. Activate switches together to open doors.' : 'Co-op run in progress. Reach the exit together.')) : 'Match in progress.';if (room.phase === 'game-over') return this.buildRematchView(room).available ? 'Round complete. Choose rematch to play again.' : 'Round complete. Waiting for opponent status to change.'; if (this.isCoOpRoom(room)) return room.players.filter((player) => player.playerId).length === 2 ? 'Co-op room ready. Game starts automatically when both players are ready.' : 'Waiting for co-op partner.'; return room.players.filter((player) => player.playerId).length === 2 ? 'Game starts automatically when both players are ready.' : room.soloMode ? 'Solo mode. Starting game…' : 'Waiting for opponent.'; }
  ...
  private createInitialRoomMatch(room: RoomRecord): MatchState { return this.isCoOpRoom(room) ? createInitialCoOpRunWithRooms(room.roomCode, Math.random, CO_OP_RUN_ROOM_COUNT) : createInitialMatchState(room.roomCode, Math.random, room.soloMode); }
  ...
  private serializeHazardViews(hazards: HazardState[]): HazardView[] {
    return hazards.map((h) => {
      const position = getHazardPosition(h);
      let totalTicks: number;
      if (h.phase === 'warning') totalTicks = h.warningTicks;
      else if (h.phase === 'active') totalTicks = h.activeTicks;
      else totalTicks = h.type === 'zone' ? h.cooldownTicks : 0;
      const ticksRemaining = Math.max(0, totalTicks - h.ticksInPhase);
      const view: HazardView = {
        id: h.id,
        hazardType: h.type === 'zone' ? 'spike' : 'sweeper',
        position: { ...position },
        phase: h.phase,
        lethal: isHazardLethal(h),
        ticksRemainingInPhase: ticksRemaining,
      };
      if (h.type === 'sweep') {
        view.path = h.path.map((p: GridPoint) => ({ ...p }));
        view.pathIndex = h.pathIndex;
      }
      return view;
    });
  }
  private serializeMonsterViews(monsters: PatrolMonsterState[]): PatrolMonsterView[] {
    return monsters.map((m) => ({
      id: m.id,
      position: { ...getMonsterPosition(m) },
      path: m.path.map((p) => ({ ...p })),
