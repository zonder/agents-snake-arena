...
interface RoomRecord {
  roomCode: string; phase: RoomPhase; createdAt: number; players: [PlayerSlot, PlayerSlot]; version: number; match: MatchState | null;
  countdown: { startedAt: number; endsAt: number; secondsRemaining: 3 | 2 | 1 | 0 } | null; teardownAt: number | null; rematch: RematchState;
  disconnectWindow: DisconnectWindow | null; pausedMatch: PausedMatchState | null;
  botSlotIndex: 0 | 1 | null; botDifficulty: BotDifficulty | null;
}
...
export class RoomService {
  ...
  createRoom(socketId: string, rawName: string): ServiceResult {
    ...
    const room: RoomRecord = { roomCode, phase: 'waiting-for-players', createdAt: issuedAt, version: 1, match: null, countdown: null, teardownAt: null,
      rematch: { requestedBySlot: { 0: false, 1: false } }, disconnectWindow: null, pausedMatch: null, botSlotIndex: null, botDifficulty: null,
      players: [
  ...
  requestRematch(socketId: string): ServiceResult { const room = this.getRoomForSocket(socketId); if (!room || room.phase !== 'game-over' || room.disconnectWindow) return { events: [this.error(socketId, 'ACTION_NOT_ALLOWED')] }; const player = room.players.find((slot) => slot.socketId === socketId); if (!player) return { events: [this.error(socketId, 'ACTION_NOT_ALLOWED')] }; if (room.rematch.requestedBySlot[player.slotIndex]) return { events: [] }; room.rematch.requestedBySlot[player.slotIndex] = true; if (room.botSlotIndex !== null) room.rematch.requestedBySlot[room.botSlotIndex] = true; room.version += 1; const events = [...this.buildLobbyStateEvents(room), ...this.buildGameStateEvents(room), ...this.buildRematchStateEvents(room)]; if (!this.areBothPlayersPresent(room) || !room.rematch.requestedBySlot[0] || !room.rematch.requestedBySlot[1]) return { events }; room.phase = 'starting'; room.version += 1; room.match = createInitialMatchState(room.roomCode); room.countdown = null; room.teardownAt = null; room.players.forEach((slot) => { slot.ready = false; }); this.clearRematchState(room); events.push(...this.beginCountdown(room, 3000)); return { events }; }
  ...
  disconnect(socketId: string): ServiceResult { return this.leaveCurrentRoom(socketId, 'disconnected'); }
  addBot(requesterSocketId: string, difficulty: BotDifficulty = 'medium'): ServiceResult { const room = this.getRoomForSocket(requesterSocketId); if (!room) return { events: [this.error(requesterSocketId, 'ACTION_NOT_ALLOWED')] }; if (room.botSlotIndex !== null) return { events: [this.error(requesterSocketId, 'BOT_ALREADY_ADDED')] }; if (room.phase !== 'waiting-for-players' && room.phase !== 'lobby') return { events: [this.error(requesterSocketId, 'ACTION_NOT_ALLOWED')] }; const emptySlot = room.players.find((player) => !player.playerId); if (!emptySlot) return { events: [this.error(requesterSocketId, 'ROOM_FULL')] }; const botSocketId = `bot:${room.roomCode}:${emptySlot.slotIndex}`; const playerId = this.allocatePlayerId(); Object.assign(emptySlot, { playerId, socketId: botSocketId, connected: true, ready: true, name: 'Bot', reconnectToken: null, disconnectedAt: null, reservationExpiresAt: null }); room.botSlotIndex = emptySlot.slotIndex; room.botDifficulty = difficulty; room.version += 1; room.phase = this.computePhase(room); this.socketToRoom.set(botSocketId, room.roomCode); this.socketToPlayer.set(botSocketId, playerId); return { events: this.afterLobbyMutation(room) }; }
  isBotSocketId(socketId: string): boolean { return socketId.startsWith('bot:'); }

  private leaveCurrentRoom(socketId: string, reason: 'left' | 'disconnected'): ServiceResult { const roomCode = this.socketToRoom.get(socketId); if (!roomCode) return { events: [] }; const room = this.rooms.get(roomCode); this.socketToRoom.delete(socketId); this.socketToPlayer.delete(socketId); if (!room) return { events: [] }; const leavingPlayer = room.players.find((slot) => slot.socketId === socketId); if (!leavingPlayer) return { events: [] }; if (this.isBotSocketId(socketId)) { room.botSlotIndex = null; room.botDifficulty = null; return this.removePlayerCompletely(room, leavingPlayer.slotIndex, 'left'); } return reason === 'left' ? this.removePlayerCompletely(room, leavingPlayer.slotIndex, reason) : this.reserveDisconnectedPlayer(room, leavingPlayer.slotIndex); }
  ...
  private afterLobbyMutation(room: RoomRecord): ClientEvent[] { const events = this.buildLobbyStateEvents(room); if (!this.canStart(room)) return events; room.match = createInitialMatchState(room.roomCode); room.version += 1; this.clearRematchState(room); events.push(...this.beginCountdown(room, 3000)); return events; }
  ...
  private buildRematchStateEvents(room: RoomRecord, message?: string): ClientEvent[] { const phase = room.phase === 'in-progress' ? null : room.phase; if (!phase) return []; return room.players.flatMap((player) => { if (!player.socketId) return []; const payload: GameRematchStatePayload = { roomCode: room.roomCode, phase, players: this.serializePlayers(room), rematch: this.buildRematchView(room, player.socketId), reconnect: this.buildReconnectView(room, player.socketId), yourSlotIndex: player.slotIndex, version: room.version, message }; return [{ type: 'game:rematch-state' as const, socketId: player.socketId, payload }]; }); }
  ...
  private serializeLobbyState(room: RoomRecord, viewerSocketId: string): LobbyStatePayload { const occupiedCount = room.players.filter((player) => player.playerId).length as 0 | 1 | 2; const allPlayersPresent = occupiedCount === 2 && room.players.every((player) => player.connected); const allReady = allPlayersPresent && room.players.every((player) => player.ready); const viewer = room.players.find((player) => player.socketId === viewerSocketId) ?? null; return { roomCode: room.roomCode, phase: room.phase, yourSlotIndex: viewer?.slotIndex ?? null, players: room.players.map((player) => ({ ...toPlayerIdentityView(player), isOccupied: Boolean(player.playerId), isReady: Boolean(player.playerId) && player.ready, isConnected: Boolean(player.connected), isReserved: Boolean(player.playerId && !player.connected && player.reservationExpiresAt), isYou: player.socketId === viewerSocketId })) as LobbyStatePayload['players'], occupiedCount, allPlayersPresent, allReady, canStart: this.canStart(room), version: room.version, message: this.getLobbyMessage(room), rematch: this.buildRematchView(room, viewerSocketId), reconnect: this.buildReconnectView(room, viewerSocketId) }; }
  private serializeGameState(room: RoomRecord, viewerSocketId?: string, countdownSecondsRemaining?: 3 | 2 | 1 | 0): GameStatePayload { const match = room.match!; const viewer = viewerSocketId ? room.players.find((slot) => slot.socketId === viewerSocketId) ?? null : null; return { roomCode: room.roomCode, phase: room.phase === 'game-over' ? 'game-over' : room.phase === 'starting' ? 'starting' : 'in-progress', yourSlotIndex: viewer?.slotIndex ?? null, tickNumber: match.tickNumber, board: match.board, food: match.food, snakes: match.snakes.map((snake) => ({ slotIndex: snake.slotIndex, body: snake.body, direction: snake.direction, alive: snake.alive, score: snake.score })) as GameStatePayload['snakes'], players: this.serializePlayers(room), foodsEaten: match.foodsEaten, tickIntervalMs: match.tickIntervalMs, countdownSecondsRemaining, paused: Boolean(room.pausedMatch), reconnect: this.buildReconnectView(room, viewerSocketId), result: room.phase === 'game-over' ? { outcome: match.result === 'draw' ? 'draw' : match.winnerSlotIndex === 0 ? 'win' : 'lose', winnerSlotIndex: match.winnerSlotIndex, deathReasons: match.deathReasons } : undefined, rematch: this.buildRematchView(room, viewerSocketId), version: room.version }; }
  ...
  private error(socketId: string, reason: LobbyErrorReason): ClientEvent { const messages: Record<LobbyErrorReason, string> = { INVALID_ROOM_CODE: 'Enter a valid four-letter room code.', ROOM_NOT_FOUND: 'Room not found.', ROOM_FULL: 'Room is full right now.', GAME_ALREADY_STARTED: 'This room is already mid-match.', ACTION_NOT_ALLOWED: 'That action is not allowed right now.', INVALID_PLAYER_NAME: 'Enter a name up to 12 characters without emoji.', BOT_ALREADY_ADDED: 'A bot is already in this room.' }; return { type: 'room:error', socketId, payload: { reason, message: messages[reason] } }; }
  ...
  private buildSessionIssuedEvent(room: RoomRecord, slot: PlayerSlot, socketId: string, issuedAt: number): ClientEvent { return { type: 'session:issued', socketId, payload: { roomCode: room.roomCode, slotIndex: slot.slotIndex, reconnectToken: slot.reconnectToken!, issuedAt, version: room.version } satisfies PlayerSessionPayload }; }
