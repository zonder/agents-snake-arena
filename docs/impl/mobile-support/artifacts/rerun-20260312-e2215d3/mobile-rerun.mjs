...
const OUT_DIR = path.resolve('docs/impl/mobile-support/artifacts/rerun-20260312-e2215d3');
...
try {
  ...
  await host.waitForFunction(() => document.getElementById('buildMarker')?.textContent?.includes('e2215d3'), { timeout: 15000 });
  await guest.waitForFunction(() => document.getElementById('buildMarker')?.textContent?.includes('e2215d3'), { timeout: 15000 });
  ...
  await host.waitForSelector('#gamePanel:not(.hidden)', { timeout: 20000 });
  await guest.waitForSelector('#gamePanel:not(.hidden)', { timeout: 20000 });
  await host.waitForSelector('#touchControls:not(.hidden)', { timeout: 15000 });
  await guest.waitForSelector('#touchControls:not(.hidden)', { timeout: 15000 });
  ...
  await host.waitForFunction(() => window.eval('latestGameState?.phase') === 'in-progress', { timeout: 15000 });
  await guest.waitForFunction(() => window.eval('latestGameState?.phase') === 'in-progress', { timeout: 15000 });
